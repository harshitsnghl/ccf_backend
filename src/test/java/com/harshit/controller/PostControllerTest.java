...
class PostControllerTest {
	...
	void shouldCreateNewPost_whenContentPhotoAndTagsAreGiven() throws Exception {
		...
		List<TagDto> postTags = List.of(new TagDto("TagOne", "add"));

		when(postService.createNewPost(POST_ONE.getContent(), postPhoto, postTags)).thenReturn(POST_ONE);

		mockMvc.perform(multipart(API_URL_PREFIX + "/posts/create").file(postPhoto)
				.param("content", POST_ONE.getContent()).param("postTags", postTagsString))
				.andExpect(status().isCreated()).andExpect(jsonPath("$.content").value(POST_ONE.getContent()));
	}
	...
	void shouldUpdatePost_whenPostIdContentPhotoAndTagsAreGiven() throws Exception {
		...
		mockMvc.perform(multipart(API_URL_PREFIX + "/posts/{postId}/update", POST_ONE.getId()).file(postPhoto)
				.param("content", POST_ONE.getContent()).param("postTags", postTagsString)).andExpect(status().isOk())
	...
	void shouldReturnOK_whenPostDeleted() throws Exception {
		...
		mockMvc.perform(post(API_URL_PREFIX + "/posts/{postId}/delete", POST_ONE.getId())).andExpect(status().isOk());
	}
	...
	void shouldReturnPost_whenPostIdIsGiven() throws Exception {
		when(postService.getPostResponseById(POST_ONE.getId())).thenReturn(new PostResponse(POST_ONE, false));

		mockMvc.perform(get(API_URL_PREFIX + "/posts/{postId}", POST_ONE.getId())).andExpect(status().isOk())
	...
	void shouldReturnListOfPostLikes_whenPostIdIsGiven() throws Exception {
		...
		when(userService.getLikesByPostPaginate(POST_ONE, 0, 5)).thenReturn(List.of(USER_JOHN, USER_JANE));

		mockMvc.perform(
				get(API_URL_PREFIX + "/posts/{postId}/likes", POST_ONE.getId()).param("page", "1").param("size", "5"))
	...
	void shouldReturnListOfPostShares_whenPostIdIsGiven() throws Exception {
		...
		mockMvc.perform(
				get(API_URL_PREFIX + "/posts/{postId}/shares", POST_ONE.getId()).param("page", "1").param("size", "5"))
	...
	void shouldReturnListOfPostComments_whenPostIdIsGiven() throws Exception {
		...
		mockMvc.perform(get(API_URL_PREFIX + "/posts/{postId}/comments", POST_ONE.getId()).param("page", "1")
				.param("size", "5")).andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(1)));
	}
	...
	void shouldReturnOK_whenPostLiked() throws Exception {
		...
		mockMvc.perform(post(API_URL_PREFIX + "/posts/{postId}/like", POST_ONE.getId())).andExpect(status().isOk());
	}
	...
	void shouldReturnOK_whenPostUnliked() throws Exception {
		...
		mockMvc.perform(post(API_URL_PREFIX + "/posts/{postId}/unlike", POST_ONE.getId())).andExpect(status().isOk());
	}
	...
	void shouldCreateNewPostComment_whenContentAndPostIdAreGiven() throws Exception {
		...
		mockMvc.perform(post(API_URL_PREFIX + "/posts/{postId}/comments/create", POST_ONE.getId()).param("content",
				COMMENT_ONE.getContent())).andExpect(status().isOk())
	...
	void shouldUpdatePostComment_whenContentCommentIdAndPostIdAreGiven() throws Exception {
		...
		mockMvc.perform(post(API_URL_PREFIX + "/posts/{postId}/comments/{commentId}/update", POST_ONE.getId(),
				COMMENT_ONE.getId()).param("content", COMMENT_ONE.getContent())).andExpect(status().isOk())
	...
	void shouldReturnOK_whenCommentDeleted() throws Exception {
		...
		mockMvc.perform(post(API_URL_PREFIX + "/posts/{postId}/comments/{commentId}/delete", POST_ONE.getId(),
				COMMENT_ONE.getId())).andExpect(status().isOk());
	}
	...
	void shouldReturnListOfCommentLikes_whenCommentIdIsGiven() throws Exception {
		...
		when(userService.getLikesByCommentPaginate(COMMENT_ONE, 0, 5)).thenReturn(List.of(USER_JOHN, USER_JANE));

		mockMvc.perform(get(API_URL_PREFIX + "/posts/comments/{commentId}/likes", COMMENT_ONE.getId())
				.param("page", "1").param("size", "5")).andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(2)));
	}
	...
	void shouldCreateNewPostShare_whenContentAndPostIdAreGiven() throws Exception {
		...
		mockMvc.perform(post(API_URL_PREFIX + "/posts/{postId}/share/create", POST_ONE.getId()).param("content",
				POST_ONE_SHARE.getContent())).andExpect(status().isOk());
	}
	...
	void shouldUpdatePostShare_whenContentAndPostIdAreGiven() throws Exception {
		...
		mockMvc.perform(post(API_URL_PREFIX + "/posts/{postId}/share/update", POST_ONE.getId()).param("content",
				POST_ONE_SHARE.getContent())).andExpect(status().isOk());
	}
	...
	void shouldReturnListOfPosts_whenTagNameIsGiven() throws Exception {
		...
		when(postService.getPostByTagPaginate(TAG_ONE, 0, 5,true))
				.thenReturn(List.of(new PostResponse(POST_ONE, false), new PostResponse(POST_TWO, false)));

		mockMvc.perform(
				get(API_URL_PREFIX + "/posts/tags/{tagName}", TAG_ONE.getName()).param("page", "1").param("size", "5"))
				.andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(2)));
	}
}